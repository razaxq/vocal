...
app.whenReady().then(async () => {
  ...
  await run(`[...document.querySelectorAll('nav button')].find(b => b.textContent === '触发方式').click()`)
  await until(`[...document.querySelectorAll('button')].some(b => b.textContent === '按住说话，松开结束')`)
  await run(`[...document.querySelectorAll('button')].find(b => b.textContent === '按住说话，松开结束').click()`)
  await until(`document.querySelector('[role="option"]') !== null`)
  await run(`[...document.querySelectorAll('[role="option"]')].find(b => b.textContent === '鼠标左右键按住说话').click()`)
  await until(`document.querySelector('[aria-label="启动延迟（秒）"]') !== null`)
  assert.equal(config.hotkey.mode, 'mouseHold')
  ...
  await run(`delayField.dispatchEvent(new FocusEvent('focusout', { bubbles: true }));
    [...document.querySelectorAll('button')].find(b => b.textContent === '鼠标左右键按住说话').click();`)
  await until(`document.querySelector('[role="option"]') !== null`)
  await run(`[...document.querySelectorAll('[role="option"]')].find(b => b.textContent === '按住说话，松开结束').click()`)
  await until(`document.body.innerText.includes('Esc')`)
  assert.equal(config.hotkey.mode, 'hold')
  assert.equal(config.hotkey.mouseHoldDelayMs, 3500)
  console.log('PASS built mouse trigger settings: selection, delay validation and persistence, no Esc hint')
