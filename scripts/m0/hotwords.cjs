...
app.whenReady().then(async () => {
  ...
  assert.deepEqual(await run(`[...document.querySelectorAll('[role="option"]')].map(b => b.textContent)`), ['中键（按下滚轮）', '左键＋中键'])
  ...
  assert.equal(config.hotkey.mouseHoldDelayMs, 3500)
  console.log('PASS built trigger settings: independent switches, middle/left-middle selection, delay validation and persistence')
