...
app.whenReady().then(async () => {
  ...
  const fullscreenKeyboard = '[role="switch"][aria-label="键盘允许在全屏应用中使用"]'
  const fullscreenMouse = '[role="switch"][aria-label="鼠标允许在全屏应用中使用"]'
  assert.equal(await run(`document.querySelector('${fullscreenKeyboard}').getAttribute('aria-checked')`), 'false')
  assert.equal(await run(`document.querySelector('${fullscreenMouse}').getAttribute('aria-checked')`), 'false')
  await run(`document.querySelector('${fullscreenKeyboard}').click()`)
  await until(`document.querySelector('${fullscreenKeyboard}').getAttribute('aria-checked') === 'true'`)
  assert.equal(config.hotkey.keyboardInFullscreen, true)
  assert.equal(config.hotkey.mouseInFullscreen, false)
  await run(`document.querySelector('${fullscreenMouse}').click()`)
  await until(`document.querySelector('${fullscreenMouse}').getAttribute('aria-checked') === 'true'`)
  await run(`document.querySelector('${fullscreenKeyboard}').click()`)
  await until(`document.querySelector('${fullscreenKeyboard}').getAttribute('aria-checked') === 'false'`)
  assert.equal(config.hotkey.keyboardInFullscreen, false)
  assert.equal(config.hotkey.mouseInFullscreen, true)
  ...
  assert.equal(config.hotkey.mouseEnabled, true)
  assert.equal(await run(`document.querySelector('${fullscreenKeyboard}').getAttribute('aria-checked')`), 'false')
  assert.equal(await run(`document.querySelector('${fullscreenMouse}').getAttribute('aria-checked')`), 'true')
  console.log('PASS built trigger settings: fullscreen preferences are independent and persist across navigation')
  await run(`document.querySelector('[role="switch"][aria-label="鼠标触发"]').click()`)
  await until(`document.querySelector('[aria-label="启动延迟（秒）"]') === null`)
  assert.equal(config.hotkey.keyboardEnabled, true)
