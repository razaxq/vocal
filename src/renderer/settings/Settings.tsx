...
function HotkeyTab({ cfg, patch }: TabProps): React.ReactElement {
  ...
  const saveDelay = async (): Promise<void> => {
    ...
    if (!delay.trim() || !Number.isFinite(ms) || ms < 100 || ms > 10000) {
      setDelayError('请输入 0.1–10 秒')
      return
    }
    try {
      if (ms !== h.mouseHoldDelayMs) await patch({ hotkey: { ...h, mouseHoldDelayMs: ms } })
      setDelayError('')
    } catch { setDelayError('保存失败，请重试') }
  }
  return (
    <Page title="触发方式">
